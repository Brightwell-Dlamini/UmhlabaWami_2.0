...
export const profiles = {
  ...
  async addStaff(args: {
    ...
    if (args.role === 'super_admin' && !isSuper) {
      throw new Error('Only super admins can add super admins.');
    }
    ...
    if (!isSuper) {
      ...
      if (organizationId !== caller.organization_id) {
        throw new Error('You can only add users to your own organisation.');
      }
    }
    ...
    if (!url || !key) {
      throw new Error(
        'Cannot add staff: Supabase env vars are missing (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY).'
      );
    }
    ...
    if (!userId) {
      throw new Error(
        'Signup did not return a user id — email confirmation may be enabled on the project. Disable email confirmation in Supabase Auth settings.'
      );
    }
