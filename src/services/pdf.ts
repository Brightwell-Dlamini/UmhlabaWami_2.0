...
async function drawHeader({
  ...
  issueDate,
  dueDate,
}: DocumentHeaderArgs): Promise<number> {
  ...
  const logoData = await loadImageDataUrl(org.logo_url);
  if (logoData) {
    try {
      const fmt = detectImageFormat(logoData);
      doc.addImage(logoData, fmt, 20, 12, 18, 18);
      textX = 42;
    } catch {
      /* ignore */
    }
  ...
  doc.text(org.company_name || 'Umhlaba Wami', textX, y);
  y += 6;
  ...
  if (org.phone || org.email) {
    doc.text([org.phone, org.email].filter(Boolean).join('  •  '), textX, y);
    y += 4;
  }
  if (org.organization_code) {
    doc.setFont('courier', 'normal');
    doc.text(`Org code: ${org.organization_code}`, textX, y);
    doc.setFont('helvetica', 'normal');
    y += 4;
  }

  if (logoData) y = Math.max(y, 34);
...
function drawTotals({
  ...
  if (typeof amountPaid === 'number') {
    ...
    doc.text(formatMoney(total - amountPaid, currency), valueX, y, {
      align: 'right',
    });
    y += 6;
  }
...
function drawFooter(doc: jsPDF, extraLines: string[] = []) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('Generated by Umhlaba Wami', pageWidth / 2, pageHeight - 8, {
      align: 'center',
    });
  }
}
...
export async function generateInvoicePdf(
  ...
  else download(doc, `Invoice-${invoice.invoice_number}.pdf`);
}

export async function generateQuotePdf(quote: Quote, org: Organization, opts?: { open?: boolean }): Promise<void> {
  const doc = new jsPDF({ unit: 'mm', format: 'a4' });
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  if (opts?.open) openPdfInNewTab(doc);
  else download(doc, `Quote-${quote.quote_number}.pdf`);
}
...
export async function generateStatementPdf(
  ...
  doc.setFontSize(11);
  doc.setTextColor(15, 23, 42);
  doc.text(formatMoney(statement.opening_balance), rightEdge, startY + 6, {
    align: 'right',
  });
  ...
  doc.setFontSize(11);
  doc.setTextColor(15, 23, 42);
  doc.text(formatMoney(statement.closing_balance), rightEdge, startY + 20, {
    align: 'right',
  });
  ...
  const periodInvoices = invoices
    .filter((i) => withinPeriod(i.issue_date, periodStart, periodEnd))
    .map((i) => ({
      date: datePart(i.issue_date),
      reference: i.invoice_number,
      description: `Invoice issued — ${i.type}`,
      debit: i.total,
      credit: 0,
      _sort: new Date(datePart(i.issue_date) + 'T00:00:00Z').getTime(),
    }));

  const periodPayments = payments
    .filter((p) => withinPeriod(p.paid_at, periodStart, periodEnd))
    .map((p) => ({
      date: datePart(p.paid_at),
      reference: p.reference ?? p.id.slice(0, 8),
      description: `Payment received — ${p.method}`,
      debit: 0,
      credit: p.amount,
      _sort: new Date(datePart(p.paid_at) + 'T00:00:00Z').getTime(),
    }));
  ...
  const stName = `Statement-${tenant.business_name.replace(
      /[^a-zA-Z0-9]/g,
      '_'
    )}-${statement.period_start}_${statement.period_end}.pdf`;
  if (opts?.open) openPdfInNewTab(doc);
  else download(doc, stName);
}

export async function generateLeaseCertificatePdf(
  lease: Lease,
  org: Organization,
  tenant?: Tenant,
  opts?: { open?: boolean }
