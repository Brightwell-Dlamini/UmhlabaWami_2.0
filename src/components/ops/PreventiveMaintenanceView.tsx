...
export function PreventiveMaintenanceView() {
  ...
  const { data: tasks = [] } = useSupabaseQuery(['pm', orgId], () => pmApi.list(), { enabled: !!orgId });
  useRealtime({ table: 'preventive_maintenance', filter: `organization_id=eq.${orgId}`, invalidateKeys: ['pm'], enabled: !!orgId });

  const add = useSupabaseMutation({
    mutationFn: () => pmApi.create({ title: title.trim(), category, frequency_days: frequency }),
    invalidateKeys: ['pm'],
    onSuccess: () => setTitle(''),
  });
      ...
      <div className="rounded-xl border bg-white dark:bg-slate-900 p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value as TicketCategory)}
            ...
            {['Air Conditioning','Electrical','Plumbing','Security','Structural Damage','Other'].map((c) => (
          ...
          <div className="flex gap-2">
            <input type="number" min={1} value={frequency}
              ...
              className="w-24 rounded-lg border bg-transparent px-3 py-2 text-sm" title="Frequency (days)" />
            <button onClick={() => add.mutate(undefined as never)} disabled={!title.trim()}
      ...
      <div className="overflow-x-auto rounded-xl border bg-white dark:bg-slate-900">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y">
            {tasks.length === 0 ? (
              <tr><td colSpan={6} className="p-8 text-center text-slate-400 text-xs">No PM tasks yet.</td></tr>
            ) : tasks.map((t) => (
              <tr key={t.id}>
                <td className="px-4 py-3">{t.title}</td>
                <td className="px-4 py-3">{t.category}</td>
                <td className="px-4 py-3">{t.frequency_days}</td>
                <td className="px-4 py-3">{new Date(t.next_due_at).toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  <span className={t.status === 'Overdue' ? 'text-red-600 font-semibold' : ''}>{t.status}</span>
                </td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => complete.mutate(t.id)}
                    className="text-blue-600 hover:underline text-xs font-medium mr-3">Mark complete</button>
                  <button onClick={() => confirm('Delete?') && remove.mutate(t.id)}
