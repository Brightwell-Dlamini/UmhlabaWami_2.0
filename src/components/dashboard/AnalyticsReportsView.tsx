...
export const AnalyticsReportsView: React.FC = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const emergencyClosed = closed.filter((t) => t.priority === 'Emergency');
    const slaCompliant = closed.filter((t) => t.sla_status === 'Compliant');
    return {
      emergencyRate: closed.length ? Math.round((emergencyClosed.length / closed.length) * 100) : 0,
      slaRate: closed.length ? Math.round((slaCompliant.length / closed.length) * 100) : 0,
      occupancy: shops.length ? Math.round((shops.filter((s) => s.status === 'Occupied').length / shops.length) * 100) : 0,
      collectionRate: invoices.length
        ? Math.round((invoices.reduce((s, i) => s + i.amount_paid, 0) / invoices.reduce((s, i) => s + i.total, 0)) * 100)
        : 0,
    };
  }, [tickets, invoices, shops]);

  const exportReport = () => {
    downloadCsv(`umhlaba-wami-sla-report-${new Date().toISOString().slice(0, 10)}.csv`, [
      ['Metric', 'Value'],
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-blue-600" /> Analytics & reports
          </h1>
          <p className="text-xs text-slate-500 mt-1">Live SLA performance, occupancy and collections</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={k.label} className="p-5 rounded-2xl bg-white dark:bg-slate-800 border">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-800 border space-y-3">
          ...
          {vendors.slice(0, 5).map((v) => (
            <div key={v.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/40 flex items-center justify-between text-xs">
              <div>
                <div className="font-bold">{v.company_name}</div>
                <div className="text-[11px] text-slate-500">{v.service_category}</div>
              </div>
              <div className="text-emerald-600 font-bold">★ {v.performance_rating}</div>
            </div>
          ))}
        </div>
      </div>
