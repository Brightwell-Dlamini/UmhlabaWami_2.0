...
export const TenantsListView: React.FC<Props> = ({
  ...
  const { data: portalUsers = [] } = useSupabaseQuery(
    ...
    () =>
      profilesApi
        .list(orgId)
        .then((list) => list.filter((u) => u.role === 'tenant')),
    { enabled: !!orgId }
  );
  ...
  const create = useSupabaseMutation({
    mutationFn: (input: Parameters<typeof tenantsApi.create>[0]) =>
      tenantsApi.create(input),
  ...
  const filtered = useMemo(
    () =>
      tenants.filter((t) => {
        ...
        if (selectedCenter !== 'All' && t.shopping_center_id !== selectedCenter)
          return false;
        if (search) {
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Occupancies, lease links, portal login — create both in one step
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Notice given',
            value: tenants.filter((t) => t.status === 'Notice Given').length,
          },
          {
            label: 'Monthly bill',
            value: `E ${tenants
...
function TenantForm({
  ...
  const availableShops = shops.filter(
    (s) => s.shopping_center_id === form.shopping_center_id
  );
  ...
  return (
    <Modal
      ...
      onClose={onCancel}
      size="md"
      title={initial ? 'Edit tenant' : 'Register tenant'}
    >
      <form
        onSubmit={async (e) => {
          e.preventDefault();
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  shopping_center_id: e.target.value,
                  shop_id: '',
                })
              }
              className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border"
            ...
            <div className="mt-3 p-3 rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/50 dark:bg-blue-950/20 space-y-2">
              ...
              {createPortal && (
                <div>
                  ...
                  />
                  <p className="text-[10px] text-slate-400 mt-1">
                    Uses the email and contact name above.
                  </p>
                </div>
              )}
              {portalError && (
