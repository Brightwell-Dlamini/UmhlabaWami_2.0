...
import {
  Ticket as TicketIcon, PlusCircle, Clock, CheckCircle2, AlertTriangle,
  FileBadge, Phone, Megaphone,
} from 'lucide-react';
...
export const TenantDashboard: React.FC<Props> = ({ onOpenCreateTicket, onViewTicket }) => {
  const user = auth.getCurrentUser();
  const orgId = user?.organization_id ?? '';

  const { data: tickets = [] } = useSupabaseQuery(['tickets', orgId], () => ticketsApi.list(), { enabled: !!orgId });
  const { data: announcements = [] } = useSupabaseQuery(['announcements', orgId], () => annApi.active(), { enabled: !!orgId });
  ...
  const myTickets = useMemo(() => tickets.filter((t) =>
    t.created_by_user_id === user?.id || (user?.shop_id && t.shop_id === user.shop_id)
  ), [tickets, user]);
    ...
    <div className="space-y-6 pb-12">
      <div className="p-6 rounded-2xl bg-gradient-to-r from-blue-700 to-indigo-900 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold mt-1">{greeting}, {user?.name}</h1>
          <p className="text-xs text-blue-100 mt-1">{tenant?.business_name ?? 'Commercial tenant'}</p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-xs text-amber-900 dark:text-amber-200">
                Action required: {awaiting} ticket{awaiting > 1 ? 's' : ''} marked Resolved
              </h4>
              <p className="text-[11px] text-amber-700 dark:text-amber-300">Please confirm or reopen.</p>
            </div>
          </div>
          <button onClick={() => onViewTicket(myTickets.find((t) => t.status === 'Resolved')!.id)}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Open', value: openCount, icon: TicketIcon },
          { label: 'In progress', value: inProgress, icon: Clock },
          { label: 'Awaiting confirm', value: awaiting, icon: AlertTriangle },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border overflow-hidden">
            {myTickets.length === 0 ? (
              <div className="p-8 text-center text-slate-400 text-xs">No tickets yet.</div>
            ) : myTickets.map((t) => (
              <div key={t.id} onClick={() => onViewTicket(t.id)}
                className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/40 cursor-pointer flex items-center justify-between gap-4 border-b last:border-0">
                ...
                <div className="text-right shrink-0">
                  <div className="text-[11px] font-semibold text-blue-600">View →</div>
                  <div className="text-[10px] text-slate-400">SLA: {t.sla_status}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {announcements.length > 0 && (
            <>
              ...
              <div className="space-y-2">
                {announcements.map((a) => (
                  <div key={a.id} className="p-4 rounded-xl bg-white dark:bg-slate-800 border">
                    <div className="flex items-center justify-between mb-1">
        ...
        <div className="space-y-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-800 border space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider">Lease</h3>
            </div>
            {lease ? (
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500">Rent</span><strong>E{lease.rental_amount.toLocaleString()}</strong></div>
                <div className="flex justify-between"><span className="text-slate-500">Start</span><strong>{lease.start_date}</strong></div>
                <div className="flex justify-between"><span className="text-slate-500">End</span><strong>{lease.end_date}</strong></div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Signed</span>
                  <strong className={lease.is_digitally_signed ? 'text-emerald-600' : 'text-amber-600'}>
          ...
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-800 border space-y-3">
            ...
            {[
              ...
              <div key={c.label} className="p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between">
                <div>
                  <div className="text-xs font-semibold">{c.label}</div>
                </div>
                <a href={`tel:${c.phone.replace(/\s/g, '')}`} className="text-xs font-bold text-blue-600">{c.phone}</a>
              </div>
            ))}
          </div>
        </div>
