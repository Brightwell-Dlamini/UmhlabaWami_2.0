...
import {
  Shield, Layers, CheckCircle2, XCircle, History, Download, RotateCcw, Store,
} from 'lucide-react';
...
export const SuperAdminPortal: React.FC<Props> = ({ initialTab }) => {
  const [activeTab, setActiveTab] = useState<'approvals' | 'organizations' | 'listings' | 'audit'>(
  ...
  const { data: logs = [] } = useSupabaseQuery(['super_audit'], () => auditApi.list(undefined as never, 200), {
  ...
  const approve = useSupabaseMutation({
    ...
    onSuccess: (res) => {
      setActionNotice(`Approved — code ${res.organizationCode}${res.inviteSent ? ' · invite email sent' : ''}.`);
      setTimeout(() => setActionNotice(''), 5000);
      setCustomCode(''); setAdminEmail('');
    },
  });
  ...
  const reject = useSupabaseMutation({
    ...
    invalidateKeys: ['super_orgs'],
    onSuccess: () => { setActionNotice('Rejected.'); setTimeout(() => setActionNotice(''), 3000); },
  });
  ...
  const exportBackup = () => {
    downloadCsv(
      ...
      [
        ...
        ...orgs.map((o) => [o.organization_code, o.company_name, o.owner_name, o.email, o.subscription_tier, o.status]),
      ]
    );
    ...
    <div className="space-y-6 pb-12">
      <div className="p-6 rounded-2xl bg-gradient-to-r from-red-800 to-slate-950 text-white shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold mt-1">Platform governance</h1>
          <p className="text-xs text-slate-300">Review applications, manage organisations, audit</p>
        </div>
      ...
      {activeTab === 'approvals' && (
        <div className="space-y-4">
          {pending.length === 0 ? (
            <div className="p-12 text-center bg-white dark:bg-slate-800 rounded-2xl border text-xs text-slate-400">
              <CheckCircle2 className="w-8 h-8 text-emerald-500 mx-auto mb-2" />
              All caught up.
            </div>
          ) : pending.map((org) => (
            <div key={org.id} className="p-6 rounded-2xl bg-white dark:bg-slate-800 border-2 border-amber-300 space-y-4">
              <div className="flex items-start justify-between gap-3 border-b pb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-base">{org.company_name}</h3>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-100 text-amber-800 uppercase">Pending</span>
                  </div>
                ...
                <div className="text-right text-xs">
                  ...
                  {org.monthly_fee_estimate && (
                    <div className="font-bold mt-1">E{org.monthly_fee_estimate.toLocaleString()}/mo</div>
                  )}
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-50 dark:bg-slate-900/50 rounded-xl border flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto text-xs">
                  <div>
                    ...
                    <input value={customCode} onChange={(e) => setCustomCode(e.target.value.toUpperCase())}
                      ...
                      className="px-2.5 py-1.5 rounded-lg border font-mono text-xs bg-white dark:bg-slate-800" />
                  </div>
                  <div>
                    ...
                    <input value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)}
                      ...
                      className="px-2.5 py-1.5 rounded-lg border text-xs bg-white dark:bg-slate-800" />
                  </div>
                </div>
                <div className="flex items-center gap-2 justify-end w-full sm:w-auto">
                  <button onClick={() => reject.mutate({ orgId: org.id })}
                    className="px-3.5 py-2 rounded-xl text-xs font-semibold text-red-600 hover:bg-red-50">
                    Reject
                  </button>
                  <button onClick={() => approve.mutate({ orgId: org.id })} disabled={approve.loading}
                    ...
                    <CheckCircle2 className="w-4 h-4" /> Approve & issue code
                  </button>
                </div>
              </div>
            </div>
                ...
                <tr key={o.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      o.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                    }`}>{o.status}</span>
                  </td>
                </tr>
              ))}
