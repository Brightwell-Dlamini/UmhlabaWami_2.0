    ...
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full border p-6 shadow-2xl space-y-4">
        ...
        >
          ...
          <div>
            <label className="block font-semibold mb-1">Password (min 8 chars)</label>
            <input
              required
              type="password"
              minLength={8}
...
function FullEditForm({
  ...
  const handleSave = () => {
    const patch: Partial<Pick<User, 'name' | 'email' | 'phone' | 'role' | 'status' | 'organization_id' | 'username'>> = {
      name,
      email,
      phone: phone || undefined,
    ...
    if (isSuper) {
      patch.organization_id = organizationId || null;
    }
    onSave(patch);
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full border p-6 shadow-2xl space-y-4">
        ...
        <div className="flex justify-between gap-2 pt-3 border-t">
          <button
            type="button"
            onClick={() => onRoleOnly(role)}
            className="px-3 py-2 rounded-xl border text-xs text-slate-600"
            title="Update role only (uses RPC when available)"
          >
            Role only
          </button>
