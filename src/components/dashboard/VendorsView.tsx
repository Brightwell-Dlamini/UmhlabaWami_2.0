...
import { Wrench, Search, Star, PlusCircle, Edit3, Trash2, X, CheckCircle2 } from 'lucide-react';
...
export const VendorsView: React.FC = () => {
  ...
  const { data: vendors = [] } = useSupabaseQuery(['vendors', orgId], () => vendorsApi.list(), { enabled: !!orgId });
  useRealtime({ table: 'vendors', filter: `organization_id=eq.${orgId}`, invalidateKeys: ['vendors'], enabled: !!orgId });

  const create = useSupabaseMutation({
    mutationFn: (input: Omit<Vendor, 'id' | 'organization_id'>) => vendorsApi.create(input),
    invalidateKeys: ['vendors'],
    onSuccess: () => { setFeedback('Vendor registered.'); setTimeout(() => setFeedback(''), 3000); setShowModal(false); },
  });
  const update = useSupabaseMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<Vendor> }) => vendorsApi.update(id, patch),
    invalidateKeys: ['vendors'],
    onSuccess: () => { setFeedback('Vendor updated.'); setTimeout(() => setFeedback(''), 3000); setEditing(null); setShowModal(false); },
  });
  const remove = useSupabaseMutation({
    mutationFn: (id: string) => vendorsApi.remove(id),
    invalidateKeys: ['vendors'],
    onSuccess: () => { setFeedback('Vendor removed.'); setTimeout(() => setFeedback(''), 3000); },
  });

  const categories = useMemo(() => Array.from(new Set(vendors.map((v) => v.service_category))), [vendors]);

  const filtered = vendors.filter((v) => {
    if (selectedCategory !== 'All' && v.service_category !== selectedCategory) return false;
    if (search) {
      const q = search.toLowerCase();
      return v.company_name.toLowerCase().includes(q) || v.service_category.toLowerCase().includes(q);
    }
    return true;
  });
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Verified suppliers and emergency callout contacts</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((v) => (
          <div key={v.id} className="p-5 rounded-2xl bg-white dark:bg-slate-800 border space-y-3 flex flex-col">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-blue-50 text-blue-700">{v.service_category}</span>
            ...
            <div className="pt-3 border-t flex gap-2">
              ...
              <button onClick={() => confirm('Delete?') && remove.mutate(v.id)}
...
function VendorForm({
  ...
  const [form, setForm] = useState({
    ...
    email: initial?.email ?? '',
    assigned_property_ids: initial?.assigned_property_ids ?? [],
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full border p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-base">{initial ? 'Edit vendor' : 'Add vendor'}</h3>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-3 text-xs">
          <input required value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })}
            placeholder="Company name" className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border" />
          <input required value={form.service_category} onChange={(e) => setForm({ ...form, service_category: e.target.value })}
            placeholder="Service category" className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border" />
          <input required value={form.contact_person} onChange={(e) => setForm({ ...form, contact_person: e.target.value })}
            placeholder="Contact person" className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border" />
          ...
          <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as never })}
            className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border">
            <option>Active</option><option>Under Review</option><option>Inactive</option>
          </select>
