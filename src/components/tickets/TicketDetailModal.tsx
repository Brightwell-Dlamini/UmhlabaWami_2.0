...
export const TicketDetailModal: React.FC<Props> = ({ ticketId, onClose, onRefresh }) => {
  ...
  const { data: ticket, loading } = useSupabaseQuery(
    ...
    () => (ticketId ? ticketsApi.get(ticketId) : Promise.reject('no id')),
    { enabled: !!ticketId }
  );
  ...
  const { data: technicians = [] } = useSupabaseQuery(
    ...
    () => profilesApi.list().then((list) => list.filter((u) => u.role === 'maintenance')),
  ...
  const reopen = useSupabaseMutation({
    mutationFn: ({ reason }: { reason: string }) => ticketsApi.reopen(ticketId!, reason),
    invalidateKeys: ['tickets', 'notifications'],
    onSuccess: () => { onRefresh?.(); onClose(); },
  });
  ...
  const addComment = useSupabaseMutation({
    ...
    invalidateKeys: ['ticket_comments', ticketId ?? ''],
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        ...
        <div className={`px-6 py-2.5 flex items-center justify-between text-xs border-b ${
          isBreached ? 'bg-red-50 dark:bg-red-950/40 text-red-800 dark:text-red-300'
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500 space-y-0.5">
              ...
              <div>Assigned: <strong>{ticket.assigned_to_name || 'Unassigned'}</strong></div>
            </div>
          </div>
          ...
          {ticket.repair_notes && (
            <div className="p-4 bg-emerald-50/60 dark:bg-emerald-950/30 rounded-xl border border-emerald-200 text-xs space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t text-[11px]">
                <div>Materials: <strong>{ticket.materials_used || '—'}</strong></div>
                <div>Time: <strong>{ticket.time_spent_hours ?? '—'} hrs</strong></div>
                <div>Cost: <strong>E{ticket.cost ?? 0}</strong></div>
              </div>
            </div>
          )}
              ...
              {!showReopenInput ? (
                <>
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => confirm.mutate({ rating, feedback })}
                    ...
                    <button onClick={() => setShowReopenInput(true)}
                ...
                <div className="p-4 bg-red-50 dark:bg-red-950/40 rounded-xl border border-red-200 space-y-3">
                  <textarea rows={2} value={reopenReason}
                    onChange={(e) => setReopenReason(e.target.value)}
                    placeholder="Why is it not fixed?"
                    className="w-full px-3 py-2 rounded-xl border text-xs" />
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowReopenInput(false)}
                      className="px-3 py-1.5 text-xs text-slate-600">Cancel</button>
                    <button onClick={() => reopen.mutate({ reason: reopenReason })}
                      disabled={!reopenReason || reopen.loading}
          ...
          {currentUser?.role === 'maintenance' &&
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/80 border space-y-3">
              ...
              {ticket.status === 'Open' ? (
                <button onClick={() => accept.mutate(undefined as never)}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input value={repairNotes} onChange={(e) => setRepairNotes(e.target.value)}
                      placeholder="Repair notes" className="px-3 py-2 rounded-xl border text-xs" />
                    <input value={materialsUsed} onChange={(e) => setMaterialsUsed(e.target.value)}
                      placeholder="Materials used" className="px-3 py-2 rounded-xl border text-xs" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <input type="number" step="0.5" value={hoursSpent}
                      onChange={(e) => setHoursSpent(e.target.value)}
                      placeholder="Hours" className="px-3 py-2 rounded-xl border text-xs" />
                    <input type="number" value={cost}
                      onChange={(e) => setCost(e.target.value)}
                      placeholder="Cost (E)" className="px-3 py-2 rounded-xl border text-xs" />
                  </div>
                  <input value={afterPhotoUrl} onChange={(e) => setAfterPhotoUrl(e.target.value)}
                    placeholder="After photo URL (optional)"
                    className="w-full px-3 py-2 rounded-xl border text-xs" />
                  <button onClick={() => resolve.mutate(undefined as never)}
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                <button
                  onClick={() => {
                    ...
                    if (tech) assign.mutate({ techId: tech.id, techName: tech.name });
                  }}
                  disabled={!selectedTechId || assign.loading}
              ...
              {ticket.timeline.map((item) => (
                <div key={item.id} className="relative">
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5">
                    {item.actor_name} ({item.actor_role}) • {new Date(item.timestamp).toLocaleString()}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border-t pt-4">
            ...
            <form
              onSubmit={(e) => { e.preventDefault(); if (newComment.trim()) addComment.mutate({ text: newComment.trim() }); }}
              className="flex gap-2">
              <input value={newComment} onChange={(e) => setNewComment(e.target.value)}
                placeholder="Type a message..."
                className="flex-1 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl border text-xs" />
              <button type="submit" disabled={!newComment.trim() || addComment.loading}
