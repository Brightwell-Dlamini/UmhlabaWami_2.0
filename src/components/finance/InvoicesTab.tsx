...
export function InvoicesTab() {
  ...
  const generate = useSupabaseMutation({
    ...
    invalidateKeys: ['invoices'],
    onSuccess: (n) => {
      setMessage(`Generated ${n} rent invoices.`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Rent generation, collections and bank reconciliation.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Plus className="w-4 h-4" /> Generate next month rent
          </button>
          <button
            type="button"
            onClick={exportCsv}
      ...
      {message && (
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200 px-3 py-2 text-sm">
          {message}
        </div>
      )}
      ...
      <div>
        <h3 className="text-sm font-semibold mb-3">Bank reconciliation</h3>
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
              <tr>
                  ...
                  <tr key={l.id}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        type="button"
                        onClick={() =>
                          l.reconciled ? unreconcile.mutate(l.id) : reconcile.mutate(l.id)
                        }
