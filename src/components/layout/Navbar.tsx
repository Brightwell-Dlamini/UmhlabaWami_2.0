...
import { Bell, User, LogOut, ChevronDown, Moon, Sun, Menu, X } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const { data: unread = [] } = useSupabaseQuery(
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-16 flex items-center justify-between gap-3">
        ...
        >
          <img src={logoSrc} alt="Umhlaba Wami"
        ...
        <div className="hidden md:flex items-center gap-2">
          {currentUser ? (
            <>
              ...
              <button onClick={onToggleDarkMode} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
            ...
            <>
              ...
              <button onClick={openLogin} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800">
                Sign in
              </button>
              <button onClick={openRegister} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700">
        ...
        <div className="flex md:hidden items-center gap-1">
          ...
          <button onClick={() => setIsMenuOpen((v) => !v)} className="p-2 rounded-lg text-slate-600 dark:text-slate-300">
        ...
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-3 space-y-2">
          {currentUser ? (
            <>
              ...
              <button onClick={() => { goDashboard(); setIsMenuOpen(false); }}
                className="w-full text-left text-sm font-medium py-2">Operations workspace</button>
