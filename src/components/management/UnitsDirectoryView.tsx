...
import {
  Building2, PlusCircle, Search, Edit3, Trash2, X, Globe, CheckCircle2,
} from 'lucide-react';
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    {shop.public_listing && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 flex items-center gap-1">
...
function ShopForm({
  ...
  const [form, setForm] = useState({
    ...
    public_listing: initial?.public_listing ?? true,
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full border p-6 shadow-2xl space-y-4">
        ...
        <form onSubmit={(e) => {
          ...
          <label className="flex items-center gap-2">
            <input type="checkbox" checked={form.public_listing}
              onChange={(e) => setForm({ ...form, public_listing: e.target.checked })} />
